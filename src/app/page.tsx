...
export default function Home() {
  ...
  const innovationEntries = entries.filter((entry) => entry.category === "Innovation for Salvation")
  ...
  return (
    <div className="min-h-screen bg-linear-to-br from-gray-50 to-indigo-50">
      ...
      {/* INNOVATION FOR SALVATION SPOTLIGHT */}
      <section id="innovation-salvation" className="py-20 sm:py-32 bg-indigo-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-12">
            <h2 className="text-3xl sm:text-4xl font-bold text-gray-900 mb-4">Innovation for Salvation Spotlight</h2>
            <div className="w-24 h-1 bg-indigo-600 mx-auto mb-6"></div>
            <p className="text-lg text-gray-600 max-w-2xl mx-auto">
              Videos and documents tagged as "Innovation for Salvation" are published directly to the archive and highlighted here.
            </p>
          </div>

          {innovationEntries.length === 0 ? (
            <div className="text-center py-12">
              <p className="text-gray-600">No innovation entries yet. Upload to the admin dashboard to get started.</p>
            </div>
          ) : (
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
              {innovationEntries.map((entry) => (
                <div key={entry.id} className="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-lg transition-shadow duration-300">
                  <div className="aspect-video bg-gray-200">{getMediaPreview(entry)}</div>
                  <div className="p-6">
                    <h3 className="text-lg font-semibold text-gray-900 mb-2">{entry.title}</h3>
                    <p className="text-gray-600 text-sm line-clamp-3 mb-4">{entry.description || "Innovation content"}</p>
                    <a
          ...
          <div className="text-center mt-10">
            <a href="/archive#" className="inline-flex items-center gap-2 text-indigo-600 underline hover:text-indigo-800">
              View all Archive content
            </a>
          </div>
        </div>
      </section>
