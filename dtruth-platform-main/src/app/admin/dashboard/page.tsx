...
type Entry = {
  ...
  description?: string
  content?: string
  category?: "Document" | "Report" | "Photo" | "Video"
...
export default function Dashboard() {
  ...
  const [entryCategory, setEntryCategory] = useState<"Document" | "Report" | "Photo" | "Video">("Document")
  ...
  const loadEntries = async () => {
    try {
      const { data, error } = await supabase
        .from("entries")
        .select("id, title, description, content, category, status, media_url, uploaded_by, created_at, updated_at")
  ...
  const selectEntry = (entry: Entry) => {
    ...
    setEntryCategory(entry.category || "Document")
    ...
    try {
      ...
      if (selectedEntryId) {
        const { error } = await supabase
          .from("entries")
          .update({ title, description, content, category: entryCategory, status: entryStatus, media_url: finalMediaUrl })
        ...
        const { error } = await supabase
          .from("entries")
          .insert({ title, description, content, category: entryCategory, status: entryStatus, media_url: finalMediaUrl })
